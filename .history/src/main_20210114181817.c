...
sbit beep = P2 ^ 2;    //蜂鸣器
uint IrTime;           //红外接收计时
uchar IrValue;      //红外接收储存
uint the_key_id;       //红外转换成无符整数
int maohao = 0;        //bool型，是否显示:
int page = 0;          //主屏翻页
int mode = 0;          //一级菜单，选中的当前菜单条目 0-3
int set_time_mode = 0; //当前设置时间是什么
uint state = 0;        //判断菜单状态
    ...
    if (IRIN == 0)   //确认是否真的接收到正确的信号
    {
        ...
        if (IRIN == 1) //如果正确等到9ms低电平
        {
            ...
            for (j = 0; j < 8; j++) //接收一组数据
            {
                ...
                {
                    IrValue |= 0x80; //1000 0000
                }
                IrTime = 0; //用完时间要重新赋值
            }
        }
        the_key_id = IrValue % 16;
        Irdo();
    }
}
//共计1,190行代码