...
uchar fm_address_W = 0xA0; //硬件地址,写入
uchar fm_address_R = 0xA1; //硬件地址，读取
//普中科技开发板: 0xA0
...
{
    iic_start();
	iic_sendByte(fm_address_W);//发送写器件地址
    iic_checkACK();
...
{    
    iic_start();              //起始信号
    iic_sendByte(fm_address_W); //控制字,写
    iic_checkACK();           //处理应答信号
    iic_sendByte(beginAddr);  //发送地址
    iic_checkACK();           //处理应答信号
    iic_start();              //发送起始信号
    iic_sendByte(fm_address_R);       //控制字,读
    iic_checkACK();           //处理应答信号
...
uchar AT24C02_readData_Byte(uchar addr)
{
    ...
    iic_checkACK();
	iic_start();
	iic_sendByte(fm_address_R); //发送读器件地址
    iic_checkACK();
