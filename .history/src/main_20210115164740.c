...
char YearFlag(uchar cYear) //判断平年，闰年
{
...
uchar Conver_WEEK(unsigned char year, unsigned char month, unsigned char day) //此函数可以屏蔽掉，星期手动调节
{                                                                             //21世纪输入输出数据均为10进制数据 										 //比较简单，因为星期数 7天一轮，
    unsigned char temp1, temp2;                                               //调一次就行了，没必要搞这么复杂
