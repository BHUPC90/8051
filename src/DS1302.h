...
uchar time[] = {0x50,0x15,0x14,0x22,0x10,0x06,0x17};                      //存放初始时间
uchar alarm_time[] = {0, 0,0};
...
void set_alarm() //在寄存器里写入初始时间
{
    uchar i;
    Ds1302Write(0x8e, 0x00); //关闭写保护寄存器
    Ds1302Write(0xC2, alarm_time[0]);
    Ds1302Write(0xC4, alarm_time[1]);
    Ds1302Write(0xC6, alarm_time[2]);
    Ds1302Write(0x8e, 0x80); //打开写保护寄存器
}

void read_alarm() //在寄存器里写入初始时间
{
    alarm_time[0] = Ds1302Read(0xc3);
    alarm_time[1] = Ds1302Read(0xc5);
    alarm_time[2] = Ds1302Read(0xc7);
}